...
function [u0,x0] = polarSCL(llr, iMap, L, POLAR_TYPE, CRC_POLY, batch)
pm_prev = 0;
N = size(llr, 2);
switch POLAR_TYPE
    case 0
    ...
    case {42, 43, 44}
        ...
        vec_punc = ones(batch, 1)*vec_punc;
        vec_punc(vec_punc == 0) = 1000;
        vec_punc(vec_punc == 1) = llr;
        [u,x,~, ~] = polarSCLIte(vec_punc, iMap, pm_prev, L);
%         [u,x,~, ~] = batchpolarSCLIte(vec_punc, iMap, pm_prev, L, 1);
    case 45
...
if(length(CRC_POLY) ~= 1)
    for b = 1:batch
        ...
        batch_i(b) = i+(b-1)*batch;
    end
else
    for b = 1:batch
        batch_i(b) = 1 + batch*(b-1);
    end
end
u0 = u(batch_i,iMap == 1);
x0 = x(batch_i, :);
...
function [u, x, pm, cidx] = batchpolarSCLIte(llr, iMap, pm_prev, L, batch)
N = size(llr, 2);   %% block length
LI = size(llr,1);   %% number of the input candidates
path_size = round(LI/batch);
p_idx = 1: LI;
if(N == 1)
    if(iMap(1) == 1)
        LLO = 2*LI;  %%% double the list size
        ...
        i = ceil(i_tmp/2);
        m = mod(i_tmp, 2);
        pmo = pm_prev(i) - maxStar((2*m-1) .* llr(i, 1).');
        pp = p_idx(i);
        xx(i_tmp, 1) = m;
    ...
    pmo_mat = zeros(round(LLO/batch), batch);
    pmo_mat(:) = pmo;
    [~ , idx_mat] = sort(pmo_mat, 'descend');
    LO = min(LLO/batch, L);
    
    idx_mat = idx_mat + LLO * ones(LLO, 1) * [0:batch-1];
    idx_mat = idx_mat(1:LO, :);
    idx = idx_mat(:);
...
[u1, x1, pm1, cidx1] = batchpolarSCLIte(llrU, iMap(1:N/2), pm_prev, L, batch); %%% perform upper SCL decoding

llrL = (1- 2 * x1) .* llr(cidx1, 1:N/2) + llr(cidx1, N/2+1: N); %%% generate the LLR for the lower SCL decoder

[u2, x2, pm, cidx2] = batchpolarSCLIte(llrL, iMap(N/2+1: N), pm1, L, batch); %%% perform lower SCL decoding
...
function [u, x, pm, cidx] = ssspolarSCLIte(llr, iMap, pm_prev, L, batch)
N = size(llr, 2);   %% block length
list_len = size(llr,1)/batch;   %% number of the input candidates 

input_idx = 1: size(llr, 1);
if(N == 1)
    if(iMap(1) == 1)
        list_len_tmp = list_len*2;  %%% double the list size
        tmp_i = 1:2*size(llr,1);
        i = ceil(tmp_i/2);
    ...
    pm_tmp_sorting_mat = zeros(list_len_tmp, batch);
    pm_tmp_sorting_mat(:) = pm_tmp;
    [~ , sorted_idx_mat] = sort(pm_tmp_sorting_mat, 'descend'); %%% sorting the path metric
    output_list_len = min(list_len_tmp, L);

    sorted_idx_mat = sorted_idx_mat + list_len_tmp*ones(list_len_tmp, 1)*[0:batch-1];
    sorted_idx_mat = sorted_idx_mat(1:output_list_len, :);
    sorted_idx = sorted_idx_mat(:);

    x(:,1) = x_with_pm_tmp(sorted_idx,1); %%% retain the bits of the best metrics
    u = x;
    pm = pm_tmp(sorted_idx);  %%% retain the path metric 
    cidx = idx_with_pm_tmp(sorted_idx); %%% retain the indices from previous surviors
    return
end
...
[u1, x1, pm1, cidx1] = ssspolarSCLIte(llrU, iMap(1:N/2), pm_prev, L, batch); %%% perform upper SCL decoding

llrL = (1- 2 * x1) .* llr(cidx1, 1:N/2) + llr(cidx1, N/2+1: N); %%% generate the LLR for the lower SCL decoder

[u2, x2, pm, cidx2] = ssspolarSCLIte(llrL, iMap(N/2+1: N), pm1, L, batch); %%% perform lower SCL decoding
